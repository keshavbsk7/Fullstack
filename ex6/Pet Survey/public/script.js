...
async function displaySurveyResults() {
    try {
        const response = await fetch('http://localhost:8080/survey/results');
        if (response.ok) {
...
function displayQuestion1Results(parentDiv, counts) {
    ...
    div.innerHTML = `<h3>Question 1: Count of each pet type</h3>`;7
...
function displayQuestion3Results(parentDiv, counts) {
    const div = document.createElement('div');
    div.innerHTML = `<h3>Question 3: Count of primary reasons for owning a pet</h3>`;
